...
close all
clc
cd /home/robot/workspaces/ur5_mpc_ursim/data_3103/
total_file_n = 50;
iter_total_file = 0;
for file_number =1:total_file_n
    fileneme = sprintf('data_%i.csv',file_number);
    if isfile(fileneme)
        low = load(fileneme);
        [len,l] = size(low);
        if len>10
            iter_total_file = iter_total_file+1;
%     fileneme = 'data_low.csv'
        low = load(fileneme);
        [len,i] = size(low);
        ...
        max_low_cont_time(file_number) = max(low_MPC_solutions(:,7));
        min_dist = low(:,91:97);
        ...
        linear_velocities = low(:,160:166);

        for i = 2:len
            if goal_positions(i,2)~=goal_positions(i-1,2)
                s_mpc = i;
                break
            end
        end
    %     s_mpc = 2;
        len_f = len-1;
        time_spent(file_number) = low(len_f,159)-low(s_mpc,159);
        t_mpc = len_f-s_mpc;
        cost = cost_calc(joint_positions(s_mpc:len_f,:), high_goal(s_mpc:len_f,:), low_MPC_solutions(s_mpc:len_f,1:6), human_sphere(s_mpc:len_f,:), t_mpc);
        mpc_cost = cost(1,:);
        mpc_jp_cost = cost(2,:);
        mpc_jv_cost = cost(3,:);
        mpc_rh_cost = cost(4,:);
    ...
    %     fig_sd = figure(2)
    %     hold on
    %     plot(smallest_distance);
    %     set(gca,'XTick',0:200:200*len);
    %     set(gca,'XTickLabel',0:dt*200:len*200*dt);
    %     saveas(fig_sd,'sd.png');
%         plot_2f(linear_velocities, lin_vel_limit, 'limit', dt, s_mpc, len_f)
        error(file_number) =0; 
        for i = s_mpc:len_f
            for j = 1:7
                if linear_velocities(i,j)>lin_vel_limit(i,j)+0.01
...
close all
clc
cd /home/robot/workspaces/ur5_mpc_ursim/src/nn_train/test_log/20220331_101020
total_file_n = 200
for file_number = 1:total_file_n
